...
class LabelingApp(QtWidgets.QMainWindow):
    ...
    def on_recognition_finished(self, predictions, status):
        ...
        if self.frame_wise_root:
            out_path = os.path.join(os.path.dirname(self.frame_wise_root), "frame_wise_recognition.json")
            try:
        ...
        self.lbl_status.setText(f"Recognition done: {status}")
    ...
    def draw_recognition_on_full_frame(self, frame, frame_idx):
        if (not self.recognition_results and not self.video_recognition_results) or not self.tracking_meta:
            return
        ...
        for item in frame_tracks:
            ...
            if not fw_pred and not vw_pred:
                continue
            x1, y1, _, _ = [int(v) for v in bbox]
            lines = []
            if fw_pred:
                lines.append(f"FW:{fw_pred['turn_label']}|{fw_pred['brake_label']}")
            if vw_pred:
                lines.append(f"VW:{vw_pred['turn_label']}|{vw_pred['brake_label']}")
            line_h = 18
            max_w = 0
            for line in lines:
            ...
            for i, line in enumerate(lines):
                cv2.putText(
                    ...
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,
                    (0, 255, 255) if line.startswith("FW") else (255, 200, 0),
                    1,
                )
